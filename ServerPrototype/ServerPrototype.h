...
#include "../P2_executive/Browse/Browse.h"
#include "../P2_executive/CheckIn/CheckIn.h"
#include "../P2_executive/CheckOut/CheckOut.h"
#include "../P2_executive/RepositoryCore/RepositoryCore.h"
#include <windows.h>
#include <tchar.h>
...
namespace Repository
{
  ...
  using ServerProc = std::function<Msg(Msg, NoSqlDb::DbCore<NoSqlDb::Payload>&)>;
  ...
  {
  public:
    Server(MsgPassingCommunication::EndPoint ep, const std::string& name, NoSqlDb::DbCore<NoSqlDb::Payload>& db);
    void start();
    void stop();
  ...
  inline Server::Server(MsgPassingCommunication::EndPoint ep, const std::string& name, NoSqlDb::DbCore<NoSqlDb::Payload>& db)
    : comm_(ep, name), db_(db) {}
    ...
    auto proc = [&]()
    {
      ...
      while (true)
      {
        ...
        if (msg.command() == "serverQuit")
          break;
		if (msg.command() == "transferFile")
			continue;
        Msg reply = dispatcher_[msg.command()](msg, db_);
